...
export function Button({ children, href, onClick, variant = 'primary', testId = 'button-action', className = '', disabled = false }: { children: ReactNode; href?: string; onClick?: () => void; variant?: 'primary' | 'secondary' | 'ghost'; testId?: string; className?: string; disabled?: boolean }) {
  ...
  return <button type="button" disabled={disabled} onClick={onClick} className={`inline-flex items-center justify-center gap-2 rounded-xl px-5 py-3 text-sm font-bold transition-all ${styles} ${disabled ? 'opacity-50 pointer-events-none' : ''} ${className}`} data-testid={testId}>{children}</button>;
}
